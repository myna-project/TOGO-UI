...
export class MeasuresService {
  ...
  getMeasures(drains: string, aggregations: string, operations: string, start: string, end: string, timeAggregation: string): Observable<any[]> {
    return this.http.get<any[][]>(this.httpUtils.getTogoAPIUrl() + this.httpUtils.getAdminUrl() + this.apiResource + '?ids%5B%5D=' + drains + '&measureAggregation%5B%5D=' + aggregations + '&operations%5B%5D=' + operations + '&start=' + start + '&end=' + end + '&timeAggregation=' + timeAggregation).pipe(
